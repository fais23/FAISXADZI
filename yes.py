...
\x1b[37m  
 ...
 |    \/    |     |
 |           |     |
 |           |     |_ _ _ _ _        Author:FAIS GANS 
                                                     :ADZI GANS
                                         FB:脳の馬鹿ファイス
                                         Github:github/fais23
...
def login():
    ...
    print(' [3] Keluar Aja terbuka kok:v')
    lg = raw_input('\n [*] Input : ')
    if lg == '':
        os.sys.exit()
    elif lg == '1' or lg == '01':
	...
	jalan (' *Anda Akan Diarahkan Ke YouTube')
	time.sleep(2)
...
def ok():
	try:
		...
		print ok
                ...
                print (' [x] Hasil Ok Lu Gada Kawan')
		os.sys.exit()
	except Exception as e:
	        print (' [x] Hasil Ok Lu Gada Kawan)
	        os.sys.exit()
def cp():
        try:
                ...
                print (' [x] Hasil Cp Lu Gada Kawan')
                os.sys.exit()
	except Exception as e:
        	print (' [x] Hasil Cp Lu Gada Kawan')
	        os.sys.exit()
...
def publik():
	...
	try:
		...
		os.rename(qq,file)
		print("\n\n [*] Selesai Ya kawan")
                print(" [*] File Dump Tersimpan Dihatimu : "+file)
                raw_input(" {Pulang Aja}")
                menu() 
...
class dump_grup:
    ...
    def extract(self, url):
        ...
        if len(self.glist) != 0:
            ...
            print ' [2] Masukan ID Grub Bukan ID EpEp/Manual'
            while True:
                c = raw_input(' [*] Input : ')
                if c == '':
                    continue
                    ...
                    print ' [x] Isi Dengan Benar Asw'

        else:
            exit(' [x] Grub Tidak Ditemukan kawan')
    ...
    def manual(self):
        ...
        if id == '':
            ...
            if 'konten tidak' in r.find('title').text.lower():
                ...
                self.f()
                print ' [*] Nama Grub Yang kamu Crack: %s' % self.listed.get('name')[0:20]
...
def metode():
    ...
    if md == '':
      ...
      ttl = raw_input('\n [*] Crack Pakai Tanggal Lahir Atau Gak  (y/n) : ')
      if ttl == '':
          menu()
      elif ttl == 'y' or ttl == 'Y':
          crack1()
...
class crack:
	def __init__(self):
		...
		self.ko=0
		while True:
			f=raw_input(" [*] Password Manual Atau Default(y/n) : ")
			if f=="":continue
			elif f=="y":
				try:
					while True:
	...
	def main(self,fl):
		try:
			...
			self.ko+=1
			print "\r\x1b[37m [Sedang TF Dosa] %s/%s *Ok : %s - *Cp : %s"%(self.ko,len(self.fl),len(self.ada),len(self.cp)),;sys.stdout.flush()
		except:
			self.main(fl)
class crack1:
	def __init__(self):
...
class crack2:
	def __init__(self):
		...
		self.ko=0
		while True:
			f=raw_input("\n [*] Pakai Password Manual Atau Default (y/n) : ")
			if f=="":continue
			elif f=="y":
				try:
					while True:
